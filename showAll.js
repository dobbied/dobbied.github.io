(function () {
    ...
    var listener = function () {
        ...
        function checkForTextElements() {
            ...
            var textHero = document.querySelectorAll('.hero-text');
            for (var i = 0; i < textHero.length; i++) {
                if (textHero[i].innerText) {
                    if (isVisible(textHero[i])) {
                        var type = "Text Visible";
                        var value = checkForChildNodes(textHero[i]);
                        if (value != false) {
                            addElementsToBox(type, value);
                        }
                    }
